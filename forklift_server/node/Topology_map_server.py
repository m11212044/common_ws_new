    ...
    def execute_cb(self, msg):
        ...
        if msg.goal != "" or (msg.target_name != "" and msg.target_pose == None):
            ...
            for i in range(len(path)):
                ...
                if (i != 0 and ((abs(waypoints[path[i]][2] - waypoints[path[i-1]][2]) >= 0.12) or (abs(waypoints[path[i]][3] - waypoints[path[i-1]][3]) >= 0.12))):
                    # 轉彎
                    dynamic_client.update_configuration({"max_vel_theta":1.48})
                    dynamic_client.update_configuration({"acc_lim_theta":0.33})
                    dynamic_client.update_configuration({"weight_max_vel_theta":0.1})
                    dynamic_client.update_configuration({"weight_acc_lim_theta":0.1})
                    dynamic_client.update_configuration({"weight_obstacle":20})
                    dynamic_client.update_configuration({"xy_goal_tolerance":0.5}) #0.33
                    dynamic_client.update_configuration({"yaw_goal_tolerance":0.06}) #0.04
                    dynamic_client.update_configuration({"min_obstacle_dist":0.9})
                    dynamic_client.update_configuration({"dt_hysteresis":0.15})
                    # dynamic_client2.update_configuration({"cost_scaling_factor":4.3})  # 6.6
                    # rospy.sleep(0.3)
                    # dynamic_client.update_configuration({"inflation_dist":3.0})
                    # dynamic_client.update_configuration({"min_obstacle_dist":0.9})
                    # dynamic_client2.update_configuration({"inflation_radius":1.5})
                    # dynamic_client2.update_configuration({"cost_scaling_factor":7.0})
                else :
                    # 直線
                    dynamic_client.update_configuration({"max_vel_theta":0.12})
                    dynamic_client.update_configuration({"acc_lim_theta":0.12})
                    # dynamic_client.update_configuration({"weight_max_vel_theta":200}) #520
                    # dynamic_client.update_configuration({"weight_acc_lim_theta":5}) #14
                    # dynamic_client.update_configuration({"weight_obstacle":350})
                    dynamic_client.update_configuration({"xy_goal_tolerance":0.058}) #0.048
                    dynamic_client.update_configuration({"yaw_goal_tolerance":0.08}) #0.085
                    # dynamic_client.update_configuration({"min_obstacle_dist":0.7})
                    dynamic_client.update_configuration({"dt_hysteresis":0.4}) #0.055
                    if "back" in path[i] :
                        dynamic_client.update_configuration({"weight_kinematics_forward_drive":0.01})
                        dynamic_client.update_configuration({"weight_max_vel_theta":50}) #520
                        dynamic_client.update_configuration({"weight_acc_lim_theta":1}) #14
                    ...
                    # dynamic_client2.update_configuration({"cost_scaling_factor":30})
                    # rospy.sleep(0.3)
                    # dynamic_client.update_configuration({"inflation_dist":0.6})
                    # dynamic_client.update_configuration({"min_obstacle_dist":0.7})
                    # dynamic_client2.update_configuration({"inflation_radius":0.7})
                    # dynamic_client2.update_configuration({"cost_scaling_factor":30.0})
